...
/**
 * 提供用于访问应用程序服务器内资源的非常简单的API：
 *
 * VFS含义是虚拟文件系统；主要是通过程序能够方便读取本地文件系统、FTP文件系统等系统中的文件资源，Mybatis中提供了VFS这个配置，主要是通过该配置可以加载自定义的虚拟文件系统应用程序。
 *
 * Mybastic的运用：例如别名注册表会通过该接口将虚拟文件系统的类注册到别名注册表中，参考：{@link org.apache.ibatis.type.TypeAliasRegistry#registerAliases(String, Class)} -> {@link ResolverUtil#find(ResolverUtil.Test, String)}
 *
 * @author Ben Gunter
 */
public abstract class VFS {
    ...
    /** {@link #addImplClass(Class)}将实现添加到的列表 */
...
//      return ReflectUtil.findClass(className);
     ...
     *
     * @param path 资源路径
     * @return 由{@link ClassLoader＃getResources（String）}返回的{@link URL}列表
     * @throws IOException If I/O errors occur
     */
